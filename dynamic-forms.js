    ...
    .directive('dynamicForm', ['$q', '$parse', '$http', '$templateCache', '$compile', '$document', '$timeout', function ($q, $parse, $http, $templateCache, $compile, $document, $timeout) {
        ...
        return {
            ...
            link: function ($scope, element, attrs) {
                ...
                if (angular.isDefined(attrs.ngModel) && (angular.isDefined(attrs.template) ||
                    angular.isDefined(attrs.templateUrl)) && !element.hasClass('dynamic-form')) {
                    ...
                    (attrs.template ? $q.when($parse(attrs.template)($scope)) :
                        ...
                        var setProperty = function (obj, props, value, lastProp, buildParent) {
                            ...
                            buildFields = function (field, id) {
                                if (String(id).charAt(0) == '$') {
                                    ...
                                    if (angular.isDefined(field.label)) {
                                        if(angular.isObject(field.label)){
                                        ...
                                        //  Everything else should be wrapped in a label tag.
                                            ...
                                            att.value =  bracket(field.model);
                                            var labelElement = $document[0].createElement('label');
                                            labelElement.setAttributeNode(att);
                                            newElement.prepend(angular.element(labelElement).html(field.label));
                                        }
                                    }
                        ...
                        && !angular.equals(iterElem[0], $document[0].documentElement)) {
